...
export const RowContextMenu: React.FC<RowContextMenuProps> = ({
  ...
  return (
    ...
    >
      ...
      <button onClick={() => onAddRow('above')} className="w-full text-left px-3 py-2 hover:bg-gray-100 text-sm flex items-center gap-2">
        <span>➕</span> Insertar fila arriba
      </button>
      <button onClick={() => onAddRow('below')} className="w-full text-left px-3 py-2 hover:bg-gray-100 text-sm flex items-center gap-2">
        <span>➕</span> Insertar fila abajo
      </button>
      <button onClick={onEditRowName} className="w-full text-left px-3 py-2 hover:bg-gray-100 text-sm flex items-center gap-2">
        <span>🏷️</span> Editar nombre de fila
      </button>
      <div className="border-t border-gray-100 my-1"></div>
      <button onClick={onDeleteRow} className="w-full text-left px-3 py-2 hover:bg-red-50 text-sm text-red-600 flex items-center gap-2">
...
export const ColumnContextMenu: React.FC<ColumnContextMenuProps> = ({
  ...
  return (
    ...
    >
      ...
      <button onClick={() => onAddColumn('left')} className="w-full text-left px-3 py-2 hover:bg-gray-100 text-sm flex items-center gap-2">
        <span>➕</span> Insertar columna izquierda
      </button>
      <button onClick={() => onAddColumn('right')} className="w-full text-left px-3 py-2 hover:bg-gray-100 text-sm flex items-center gap-2">
        <span>➕</span> Insertar columna derecha
      </button>
      <button onClick={onRenameColumn} className="w-full text-left px-3 py-2 hover:bg-gray-100 text-sm flex items-center gap-2">
